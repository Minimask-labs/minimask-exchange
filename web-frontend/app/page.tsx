      ...
      <GlassCard className="p-4">
        ...
        <div className="mb-4">
            ...
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-lg font-semibold">
                  {getBestRoute().toAmount}
                </span>
                <span className="text-success text-sm">
                  {getBestRoute().percentageDiff}
                </span>
              </div>
              <span className="text-sm text-muted-foreground">
                ≈ ${getBestRoute().toAmountUsd}
              </span>
            </div>
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              <span className="flex items-center gap-1">
                <Fuel className="w-3 h-3" />${getBestRoute().gasCostUsd}
              </span>
          ...
          {mockRoutes.length > 1 && (
            ...
            >
              {showAllRoutes ? 'Hide' : 'Show'} {mockRoutes.length - 1} more
              routes
            </button>
          )}
          ...
          {showAllRoutes && (
            <div className="space-y-2 mt-2">
              {mockRoutes.slice(1).map((route) => (
                <div
                  key={route.id}
                  className="p-3 rounded-xl bg-secondary/30 border border-minimask-border hover:border-primary/30 transition-colors cursor-pointer"
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <TokenIcon
