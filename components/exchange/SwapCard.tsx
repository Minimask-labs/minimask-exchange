...
import { useAleoSwapExecution } from '@/hooks/useAleoSwapExecution';
...
const SwapCard = () => {
  ...
  const { executeSwap, isConnected } = useAleoSwapExecution();
  ...
  return (
    <>
      <div>
        <button onClick={executeSwap}>Test Wallet</button>
      </div>
      <GlassCard className="p-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold">Exchanged</h2>
        ...
        {!isAnyWalletConnected ? (
          <GradientButton fullWidth size="lg" onClick={() => handleConnect?.()}>
